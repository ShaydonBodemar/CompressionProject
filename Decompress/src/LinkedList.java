...
public class LinkedList
{
    ...
    public int ContainsCode(int code)
    {
        for (int i = 1; i <= numItems; i++){
            if(code == RetrieveCodeAt(i)){
                return i;
            }
        }
        return -1;
    ...
    public String RetrieveTextAt(int position) throws IndexOutOfBoundsException
    {
        if (position >= 1 && position <= numItems)
        {
            Node curr = Locate(position);
    ...
    public void ListInsert(int code, String entry)
    {
        Node newNode = new Node(code, entry);
        if(head == null)
        {
            head = newNode;
            numItems++;
